...
def test_ask_llm_delegates_to_llm_resolver(monkeypatch):
    schema = {"type": "object", "properties": {"id": {"type": "string"}}, "required": ["id"]}
    seen = {}

    def fake_ask_llm(prompt, schema, images=None):
        seen.update({
            "prompt": prompt,
            "schema": schema,
            "images": images,
    ...
    monkeypatch.setattr(helpers, "_ask_llm", fake_ask_llm)
    assert helpers.ask_llm("Pick a row", schema) == {"id": "row-1"}
    assert seen == {
        "prompt": "Pick a row",
        "schema": schema,
        "images": None,
...
def test_ask_llm_forwards_images_to_llm_resolver(monkeypatch):
    schema = {"type": "object"}
    seen = {}

    def fake_ask_llm(prompt, schema, images=None):
        seen.update({"prompt": prompt, "schema": schema, "images": images})
        return {"ok": True}

    monkeypatch.setattr(helpers, "_ask_llm", fake_ask_llm)
    assert helpers.ask_llm("Inspect", schema, images=["x.png"]) == {"ok": True}
    assert seen == {"prompt": "Inspect", "schema": schema, "images": ["x.png"]}


def test_ask_gemini_is_not_exported():
    assert not hasattr(helpers, "ask_gemini")
